...
/* part 1
bool validCubeConfig(string cubeConfig)
{
    string[] foundCubes = cubeConfig.Split(",");
    foreach (string cubeInfo in foundCubes)
    {
        string[] numberType = cubeInfo.Trim().Split(" ");
        if (int.Parse(numberType[0]) > maxCubes[numberType[1]])
        {
            return false;
        }
    }
...
{
    string[] cubeConfigs = game.Split(":")[1].Trim().Split(";");
    foreach (string cubeConfig in cubeConfigs)
    {
        if (!validCubeConfig(cubeConfig))
        {
            return false;
        }
    }
...
for (int i = 0, gameNumber = i + 1; i < lines.Length; i+=1, gameNumber = i + 1)
{
    if (validGame(lines[i]))
    {
...
void setMinCubes(Dictionary<string, int> minCubes, string game)
{
    string[] cubeConfigs = game.Split(":")[1].Trim().Split(";");
    foreach (string cubeConfig in cubeConfigs)
    {
        ...
        {
            string[] numberType = cubeInfo.Trim().Split(" ");
            if (int.Parse(numberType[0]) > minCubes[numberType[1]])
            {
                minCubes[numberType[1]] = int.Parse(numberType[0]);
            }
        }
    }
}
...
for (int i = 0, gameNumber = i + 1; i < lines.Length; i += 1, gameNumber = i + 1)
{
    ...
    setMinCubes(minCubes, lines[i]);

    accumulator += minCubes["red"] * minCubes["green"] * minCubes["blue"];
}
